...
RUNTIME = ROOT / "runtime_v10_7"


def value(label: str, item: object) -> None:
    print(f"{label:<48} {item}")
...
def main() -> int:
    print("\n######## TradingBot V10.7 - LIVE FLOW CHECK ########")
    value("integrity patch", BinanceMarketDataV10.integrity_version)
    value("runtime", RUNTIME)
    value("direction prerequisite", ">=2 consecutive completed 3m signals")
    value("flow watch", "180s / 1s persistent alignment")
    value("flow threshold", ">=20 trades/60s and |imbalance|>=0.05")
    value("anti-chase", "cancel after >20bps favorable move")
    value("target gate", ">=30bps room and >=0.75 estimated net R")
    value("structural breakout watch", "RETAINED from V10.6")
    ...
    if not diagnostics.exists():
        print("\nNo V10.7 runtime yet. Start `python terminal_v10_7.py` first.")
        return 2
    ...
    print("\n######## INTERPRETATION GATE ########")
    print("V10.7 is a new forward-paper experiment; prior runtimes are not combined.")
    print("Require >=30 closed trades across >=10 active UTC dates, PF>1.10,")
    print("positive net after costs, and inspect LIVE-FLOW entries separately.")
    return 0
