# V8 S/R TP - zoom/pan + graceful shutdown + separate persistence
import sys
import asyncio
import traceback
import csv
...
class TradingWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("TradingBot V8 - SUPPORT/RESISTANCE TP")
        self.resize(650, 600)
        ...
        self.title_label = QLabel("TradingBot V8 - SUPPORT/RESISTANCE TP")
        ...
        self.tp_method_label = QLabel("TP Method: --")
        self.layout.addWidget(self.tp_method_label)

        self.rr_label = QLabel("Risk/Reward: --")
    ...
    def update_position(self, position):
        ...
        try:
            ...
            tp_method = position.get(
                "tp_method",
                "FIXED_2R"
            )

            risk_reward = position.get(
                "risk_reward",
                None
            )
            ...
            if risk_reward is None:
                ...
                self.rr_label.setText(
                    f"Risk/Reward: 1:{float(risk_reward):.2f}"
                )
...
class TradingBot:
    ...
    def __init__(self, window):
        ...
        # TRADE LOG
        # ==================================================
        self.trade_log_file = Path("trades_sr.csv")
        ...
        self.active_position_file = Path("active_position_sr.json")
    ...
    # ==========================================================
    # SUPPORT / RESISTANCE TAKE-PROFIT
    # ==========================================================

    def calculate_sr_take_profit(
        self,
        signal,
        entry_price,
        stop_loss,
        ...
        """
        Use the nearest meaningful swing support/resistance as take profit.

        LONG  -> nearest swing resistance above entry.
        SHORT -> nearest swing support below entry.

        To avoid extremely tiny targets, the level must be at least 0.75R
        away. Targets beyond the normal 2R target are ignored. If no valid
        swing level exists, fall back to the original fixed 2R target.
        """

        try:
            candles = self.candle_engine.get_candles() or []

            if len(candles) < (pivot_span * 2 + 5):
                return float(fallback_take_profit), "FIXED_2R"

            recent = candles[-lookback:]
            swing_highs = []
            swing_lows = []

            for i in range(pivot_span, len(recent) - pivot_span):
                candle = recent[i]
                high = float(candle["high"])
                low = float(candle["low"])

                left = recent[i - pivot_span:i]
                right = recent[i + 1:i + pivot_span + 1]

                nearby_highs = [float(c["high"]) for c in left + right]
                nearby_lows = [float(c["low"]) for c in left + right]
            ...
            fallback = float(fallback_take_profit)
            risk_distance = abs(entry - stop)

            if risk_distance <= 0:
                return fallback, "FIXED_2R"

            min_distance = risk_distance * float(minimum_rr)
            max_distance = risk_distance * float(maximum_rr)

            if signal == "LONG":
                candidates = sorted({
                    level for level in swing_highs
                    if entry + min_distance <= level <= entry + max_distance
                })

                if candidates:
                    return float(candidates[0]), "RESISTANCE"

            elif signal == "SHORT":
                candidates = sorted({
                    level for level in swing_lows
                    if entry - max_distance <= level <= entry - min_distance
                }, reverse=True)

                if candidates:
                    return float(candidates[0]), "SUPPORT"
    ...
    async def process_signal(
        ...
        try:
            ...
            take_profit, tp_source = (
                self.calculate_sr_take_profit(
                    signal=signal,
                    entry_price=entry_price,
                    stop_loss=stop_loss,
            ...
            reward_distance = abs(
                float(take_profit) - float(entry_price)
            )
            ...
            position["tp_method"] = tp_source
            position["risk_reward"] = rr_value
