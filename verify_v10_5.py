...
RUNTIME = ROOT / "runtime_v10_5"


def value(label: str, item: object) -> None:
    print(f"{label:<42} {item}")
...
def main() -> int:
    print("\n######## TradingBot V10.5 - STRUCTURE / REVERSAL CHECK ########")
    value("integrity patch", BinanceMarketDataV10.integrity_version)
    value("runtime", RUNTIME)
    value("actual entry", "2 signals + aligned 60s flow")
    value("actual reversal", "EXIT TO NEUTRAL; one-bar cooldown")
    value("target gate", ">=30bps room and >=0.75 estimated net R")
    value("target selection", "nearest of 2R / confirmed structure / 2 ATR")
    ...
    if not diagnostics.exists():
        print("\nNo V10.5 runtime yet. Start `python terminal_v10_5.py` first.")
        return 2
    ...
    print("\n######## INTERPRETATION GATE ########")
    print("V10.5 is a fresh forward-paper experiment; do not combine its P/L with V10.4.")
    print("Require >=30 closed trades across >=10 active UTC dates, PF>1.10,")
    print("positive net P/L after costs, and improvement in signal-to-neutral exits.")
    print("A lower trade count is expected because structurally cramped entries are rejected.")
    return 0
